...
import { IoChatboxEllipsesOutline, IoHomeOutline, IoMapOutline, IoPersonCircleOutline } from "react-icons/io5";
    ...
    <footer className="footer">
      <nav className="footer__nav">
        {tabs.map(tab => {
          ...
          if (isParams) {
            return (
              <Link key={tab.id} to={tab.path} params={tab.params} className="footer__link">
                <Icon className={`footer__${isActive ? 'active-icon' : 'icon'}`} />
                <span className={`footer__${isActive ? 'active-text' : 'text'}`}>{tab.label}</span>
              </Link>
            )
          } else {
            return (
